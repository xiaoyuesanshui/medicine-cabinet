...
function renderMedicineList(medicines) {
    ...
    container.innerHTML = medicines.map((group, index) => {
        ...
        // AI识别标识
        const aiBadge = med.is_ai_generated 
...
async function saveMedicine() {
    ...
    const data = {
        ...
        is_ai_generated: form.dataset.aiGenerated === 'true'
    };
...
async function openDetailModal(id) {
    ...
    try {
        ...
        const aiWarning = med.is_ai_generated 
            ? `<div class="ai-warning-banner">⚠️ 本页资料由AI视觉识别填写，请注意核查关键信息（药品名称、用法用量、有效期等）</div>` 
            : '';
