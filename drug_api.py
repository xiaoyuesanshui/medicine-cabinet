...
def query_by_name(name: str) -> Dict:
    """通过药品名称查询药品详情（用query接口搜，再用detail取完整信息）"""
    if not name:
        return {'success': False, 'data': None, 'error': '药品名称为空'}
    
    # 用 query 接口按名称搜索
    result = _get(JISU_QUERY_URL, {'name': name})
    if not result['success'] or not result['data']:
        return result
    
    items = result['data']
    if not isinstance(items, list) or not items:
        return {'success': False, 'data': None, 'error': f'未找到名称为"{name}"的药品'}
